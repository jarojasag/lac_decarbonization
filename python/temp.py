...
class SISEPUEDEOutputDatabase:
	...
	- dict_derivative_table_functions: optional dictionary used to specify
		...
		would be defined using the following form:
	...
	- table_name_analysis_metadata: table name to use for storing analysis metadata
		* To exclude from the output database, include "analysis_metadata" in
			`tables_write_exclude`
	...
	- fields_index: optional fields used to index the table. This can be used to
		...
		* If using CSVs, will disallow writing to the file if an index is already
			specified.
			* CAUTION: if the CSV is manipulated while a table session is
				is running, the index will not be automatically renewed. If writing
				to the table from another program, you must re-run
				`IterativeDatabaseTable._initialize_table_indices()` to ensure the
				index is up to date.
		* Avoid specifying many fields when working with large tables to preserve
			memory requirements.
	...
	- replace_on_init: replace a table existing at the connection on initialization?
